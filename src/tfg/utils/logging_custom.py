...
from src.tfg.paths import DATA_DIR, TEST_EXECUTIONS_DIR
from src.tfg.scripts.User_generator import update_users_json
from src.tfg.utils.evaluation import calcular_minutos_ponderados
...
def last_generation_update(population, num_generations, args):
    ...
    if num_generations == args.get('max_generations') - 1:
        ...
        users_data = load_users_from_json(DATA_DIR / "users.json")
...
def observador_spea2(algorithm, population, num_generations, num_evaluations, args):
    ...
    if num_generations == args.get('max_generations') - 1:
        ...
        for idx, ind in enumerate(archive):
            output_path = TEST_EXECUTIONS_DIR / "SPEA2" / f"pareto_solution_{idx}_SPEA2.json"
...
def observer_paco(paco, iteration, args, export_dir=TEST_EXECUTIONS_DIR / "PACO"):
    """
